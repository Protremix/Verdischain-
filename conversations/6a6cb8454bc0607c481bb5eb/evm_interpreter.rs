...
pub fn execute(ctx: &mut ExecutionContext) -> ExecResult {
    ...
    loop {
        ...
        match opcode {
            ...
            // Comparison & Bitwise
            0x10 => { let a = pop!(); let b = pop!(); push!(U256::from(b < a)); pc += 1; }
            0x11 => { let a = pop!(); let b = pop!(); push!(U256::from(b > a)); pc += 1; }
            0x12 => {
                let a = pop!(); let b = pop!();
                let sa = b.as_sint(); let sb = a.as_sint();
                push!(U256::from(sa < sb));
                pc += 1;
            }
            0x13 => {
                let a = pop!(); let b = pop!();
            ...
            0x14 => { let a = pop!(); let b = pop!(); push!(U256::from(a == b)); pc += 1; }
            0x15 => { let a = pop!(); push!(U256::from(a.is_zero())); pc += 1; }
            ...
            0x1D => {
                ...
                if shift >= U256::from(256u32) {
                    let sv = val.as_sint();
                    if sv < 0 { push!(U256::MAX); } else { push!(U256::zero()); }
                } else {
                    let s: u32 = shift.try_into().unwrap_or(0);
                    let sv = val.as_sint();
                    let shifted = sv >> s;
                    push!(U256::from(shifted));
                }
                pc += 1;
            }
            ...
            // Environment
            0x30 => { push!(U256::from(ctx.address.as_bytes())); pc += 1; }
            0x31 => { push!(ctx.balance); pc += 1; }
            0x32 => { push!(U256::from(ctx.origin.as_bytes())); pc += 1; }
            0x33 => { push!(U256::from(ctx.caller.as_bytes())); pc += 1; }
            ...
            0x41 => { push!(U256::from(ctx.coinbase.as_bytes())); pc += 1; }
            0x42 => { push!(U256::from(ctx.block_timestamp)); pc += 1; }
            0x43 => { push!(U256::from(ctx.block_number)); pc += 1; }
            0x44 => { push!(U256::from(ctx.prev_randao.as_bytes())); pc += 1; }
