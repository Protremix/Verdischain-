...
use verdis_runtime::{
    AccountId, AuraConfig, BalancesConfig, GenesisConfig, DposConfig, EcoConfig,
...
impl VerdisChainSpec {
    pub fn chain_spec() -> ChainSpec {
        let wasm = WASM_BINARY.expect("WASM binary not available");
        ...
        let aura_authorities: Vec<AuraId> = validators
            .iter()
            .map(|v| AuraId::from(sr25519::Public::from_raw(
                sp_core::crypto::AccountId32::from(v.clone()).into(),
            )))
            .collect();
            ...
            move || {
                GenesisConfig {
                    ...
                    balances: BalancesConfig {
                        balances: vec![
                            (root.clone(), 15_000_000_000 * UNITS), // 15B circulating
                    ...
                    dpos: DposConfig {
                        validators: validators.iter().map(|v| {
                            (v.clone(), 1_000_000_000 * UNITS, true) // address, stake, active
                        }).collect(),
                        validator_count: 5,
                    ...
                    tokenomics: TokenomicsConfig {
                        ...
                        presale_price: 500, // $0.0005 in basis points
                    },
...
const UNITS: u128 = 1_000_000_000; // 1 VRDX = 10^9 base units (nano)

type AuraId = <sc_consensus_aura::sr25519::AuthorityPair as sp_consensus_aura::AuthorityId>::;

fn get_account_id_from_seed<TPublic: Public + From<sp_core::sr25519::Public>>(
    seed: &str,
) -> AccountId
where
    TPublic::Pair: Pair,
{
    let pair = TPublic::Pair::from_string(&format!("//{}", seed), None).unwrap();
    let public = pair.public();
    AccountId::from(public.to_raw_vec())
}

fn get_from_seed<TPublic: Public>(seed: &str) -> TPublic::Public
where
    TPublic::Pair: Pair,
{
