...
for i in range(NEW_COUNT):
    ...
    # Generate ed25519 keypair for grandpa from deterministic seed
    ...
    print(f"  V{n}: {address}")
    print(f"    babe: 0x{babe_key.hex()}")
    print(f"    grandpa: 0x{grandpa_key.hex()}")
    
    new_validators.append({
        "n": n, "keypair": kp, "address": address,
        "babe_key": babe_key, "grandpa_key": grandpa_key
    })
