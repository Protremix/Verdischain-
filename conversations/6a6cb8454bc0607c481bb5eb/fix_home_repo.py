#!/usr/bin/env python3
"""
Rewrite home_repository_impl.dart to use REAL RPC data instead of hardcoded demo data.
No design changes — just fixes the data source.

Uses:
- TX Relay API (POST /api/tx-relay action=balance) for real balance
- RPC (state_getStorage) for network status (block height, peers)
- Returns empty/zero for data that can't be fetched (no fake data)
"""

REPO_CODE = r'''import 'dart:convert';
...
with open('/opt/verdis-wallet/lib/features/home/data/home_repository_impl.dart', 'w') as f:
    f.write(REPO_CODE)
print("home_repository_impl.dart rewritten with real RPC data (no demo data)")
