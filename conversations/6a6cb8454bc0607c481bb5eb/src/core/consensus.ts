...
export class DPoSConsensus {
  ...
  importState(data: {
    ...
    if (data.roundTurn !== undefined) {
      this.roundTurn = data.roundTurn;
    }
  }
}
...
export class Blockchain {
  ...
  exportState(): {
    ...
    const consensusState = this.consensus.exportState();
    const tokenState = this.tokenSystem.exportState();
    return {
      chain: this.chain,
      mempool: this.mempool.exportState(),
      validators: consensusState.validators,
      stakes: consensusState.stakes,
      balances: tokenState.balances,
      staked: tokenState.stakes,
      totalSupply: tokenState.totalSupply,
      maxSupply: tokenState.maxSupply,
      blockReward: tokenState.blockReward,
      roundTurn: consensusState.roundTurn,
    };
  }
  ...
  importState(data: {
    ...
    if (data.chain && Array.isArray(data.chain) && data.chain.length > 0) {
      this.chain = data.chain;
    }
    if (data.mempool) {
      this.mempool.importState(data.mempool);
    }
    this.consensus.importState({
      validators: data.validators || data.consensus?.validators,
      stakes: data.stakes || data.consensus?.stakes,
      roundTurn: data.roundTurn !== undefined ? data.roundTurn : data.consensus?.roundTurn,
    });
    this.tokenSystem.importState({
      balances: data.balances || data.tokenSystem?.balances,
      stakes: data.staked || data.tokenSystem?.stakes,
      totalSupply: data.totalSupply !== undefined ? data.totalSupply : data.tokenSystem?.totalSupply,
      maxSupply: data.maxSupply !== undefined ? data.maxSupply : data.tokenSystem?.maxSupply,
      blockReward: data.blockReward !== undefined ? data.blockReward : data.tokenSystem?.blockReward,
    });
  }
}
