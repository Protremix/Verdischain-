...
export const OPCODES = {
  ...
  LTE: 0x16,    // pop two, push 1 if first <= second
  GTE: 0x17,    // pop two, push 1 if first >= second
  NEQ: 0x18,    // pop two, push 1 if first !== second
  NOT: 0x19,    // pop one, push 1 if falsy, 0 if truthy
  AND: 0x1A,    // pop two, push 1 if both truthy
  OR: 0x1B,     // pop two, push 1 if either truthy
  REVERT: 0x1C, // revert execution with error
} as const;
...
export class StackVM {
   ...
   * Returns 0 for uninitialized slots to mirror standard blockchain VM storage.
   */
    ...
    while (this.pc < bytecode.length && !this.halted) {
      ...
      try {
        switch (opcode) {
          ...
          case OPCODES.REVERT: {
            const msg = this.stack.length > 0 ? this.pop() : 'Execution reverted';
            return {
              result: this.peek(),
              events: this.events,
              gasUsed: this.gasUsed,
...
function getInstructionSize(tokens: string[]): number {
  ...
  if (op === 'PUSH') return 2;
  if (['JUMP', 'JUMPI', 'LOAD', 'STORE', 'SLOAD', 'SSTORE', 'LOG', 'EMIT', 'CALL', 'REVERT'].includes(op)) {
...
export function compileContract(source: string): number[] {
  ...
  for (const { tokens } of parsedInstructions) {
    ...
    if (opStr === 'PUSH') {
      ...
      ['JUMP', 'JUMPI', 'LOAD', 'STORE', 'SLOAD', 'SSTORE', 'LOG', 'EMIT', 'CALL', 'REVERT'].includes(opStr) &&
      tokens.length > 1
    ) {
      const arg = tokens[1];
      bytecode.push(OPCODES.PUSH, parseArg(arg, labels));
