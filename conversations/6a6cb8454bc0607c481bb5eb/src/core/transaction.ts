...
export class TransactionBuilder {
  ...
  public setPublicKey(pubKey: string): this {
...
export function validateTransaction(tx: Transaction, balances: Map<string, number>): { valid: boolean; error?: string } {
  ...
  if (tx.signature && tx.signature !== 'unsigned') {
    const valid = verify(payload, tx.signature, tx.publicKey || tx.from);
    if (!valid) {
...
export class Mempool {
  ...
  /**
   * Exports all pending transactions in the mempool.
   */
  ...
  /**
   * Restores pending transactions in the mempool.
   */
  importState(txs: Transaction[]): void {
    this.clear();
