...
fn test_total_supply_conservation() {
    ...
    let allocated = 30 * billion + 20 * billion + 15 * billion + 10 * billion
        + 10 * billion + 5 * billion + 3 * billion + 2 * billion + 5 * billion;

    assert_eq!(allocated, total_supply, "Genesis allocation must equal total supply");
}
...
fn test_vesting_conservation() {
    ...
    assert_eq!(seed_vested, seed_allocation, "Seed: vested + remainder must equal allocation");
    ...
    assert_eq!(team_vested, team_allocation, "Team: vested + remainder must equal allocation");
}
...
fn test_dpos_participation_bounds() {
    ...
    assert!(total_stake >= 10 * billion, "Total stake must be >= 10% of supply");
    assert!(total_stake <= 80 * billion, "Total stake must be <= 80% of supply");
}
...
fn test_dex_constant_product_invariant() {
    ...
    assert!(k_after >= k_before, "DEX invariant: k_after must be >= k_before");
}
...
fn test_tge_circulating_supply() {
    ...
    let circulating = 10 * billion + 5 * billion + 21 * 10_000 * units;
    assert!(circulating >= 15 * billion, "TGE circulating should be >= 15B");
    assert!(circulating <= 20 * billion, "TGE circulating should be <= 20B");
}
