...
# Replace the sequential loadTransfers with a parallel version
old_func_start = '''// ===== Token Transfers =====
      ...
      try {
        ...
        for (var i = 0; i < block.block.extrinsics.length; i++) {
          ...
          if (ext.method) {
            ...
            // Also check utility.batch with transfers
...
async function scanBlockForTransfers(blockNum) {
  try {
    ...
    for (var i = 0; i < block.block.extrinsics.length; i++) {
      ...
      // System remarks (bot transactions)
      if (pallet === 'system' && method === 'remark') {
...
if old_func_start in content:
    ...
    print("Replaced loadTransfers with parallel version")
else:
    print("ERROR: old loadTransfers not found")
