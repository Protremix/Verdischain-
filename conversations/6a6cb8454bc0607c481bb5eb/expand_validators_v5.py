...
# Use HTTP (no wait_for_inclusion)
...
def submit(call_module, call_function, call_params, keypair):
    """Submit extrinsic without waiting for inclusion (HTTP mode)."""
    ...
    # Submit without waiting (HTTP doesn't support watch)
...
for v in new_validators:
    try:
        ...
        if free > FUND_AMOUNT:
            ...
            continue
        result = submit("Balances", "transfer_allow_death", {
            "dest": v["address"],
            "value": FUND_AMOUNT
        }, CHARLIE)
        print(f"  V{v['n']}: Funded {FUND_AMOUNT / 10**DECIMALS:.0f} VRDX (tx: {str(result)[:40]})")
    except Exception as e:
        print(f"  V{v['n']}: Fund error - {e}")

print("\nWaiting 15s for blocks..."); time.sleep(15)
...
for v in new_validators:
    try:
        ...
        if is_val and is_val.value:
            ...
            continue
        result = submit("Dpos", "register_validator", {
...
for v in new_validators:
    try:
        result = submit("Session", "set_keys", {
...
print("\nWaiting 20s for blocks..."); time.sleep(20)
