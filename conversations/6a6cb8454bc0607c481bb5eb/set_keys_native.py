#!/usr/bin/env python3
"""Set session keys for V22-V30 via Sudo+System::set_storage (native runtime)."""
...
def submit_sudo_set_storage(key_bytes, value_bytes):
    """Build and submit Sudo::sudo(System::set_storage([(key, value)]))"""
    # System(0)::set_storage(4) + items
    inner = bytes([0, 4]) + compact(1)  # 1 item
    ...
    body = bytes([0x84, 0x00]) + bytes(sudo_kp.public_key) + bytes([0x01]) + signature
...
for n in range(22, 31):
    ...
    try:
        ...
        if "result" in result:
            print(f"V{n}: OK (nonce={nonce}, hash={result['result'][:16]}...)")
            success += 1
        else:
    ...
    time.sleep(6)  # Wait for each tx to be included
...
for n in range(22, 31):
    ...
    if nk and nk.value:
        babe = str(nk.value.get("babe", ""))[:20]
        gp = str(nk.value.get("grandpa", ""))[:20]
        print(f"    babe: {babe}...")
        print(f"    grandpa: {gp}...")
