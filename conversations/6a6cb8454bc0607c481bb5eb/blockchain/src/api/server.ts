...
import { Blockchain, TokenSystem, DPoSConsensus } from '../core/consensus';
import { WalletManager } from '../wallet/wallet';
import { ContractManager, compileContract } from '../core/vm';
import { generateKeyPair, getPublicKeyFromPrivateKey, getAddressFromPublicKey } from '../crypto';
...
export class BlockchainAPI {
  ...
  private setupRoutes(): void {
    ...
    // ===== DEX =====
    if (this.dex) {
      this.app.get('/api/dex/pools', (req: Request, res: Response) => {
      ...
      this.app.get('/api/dex/quote', (req: Request, res: Response) => {
        ...
        const result = this.dex.quoteSwap(tokenIn as string, tokenOut as string, parseFloat(amountIn as string));
        res.json(result);
      });
    ...
    // ===== Eco Features =====
    if (this.eco) {
      this.app.get('/api/eco/impact', (req: Request, res: Response) => {
