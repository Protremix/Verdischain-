...
export class BlockchainAPI {
  ...
  private marketTracker: any = null;
  ...
  private requireAdminAuth(req: Request, res: Response, next: any): void {
    const apiKey = req.headers['x-api-key'] as string || req.query.apiKey as string;
    if (!apiKey || !this.security.verifyApiKey(apiKey)) {
      this.security.logEvent('auth_failed', 'warning', 'Admin auth failed', 
  ...
  private strictRateLimit(req: Request, res: Response, next: any): void {
  ...
  public setMarketTracker(mt: any): void {
...
<div class="container">
  ...
  <div class="footer">
    ...
    this.app.get('/api/public-url', (req: Request, res: Response) => {
      const host = req.get('host') || '';
      const protocol = req.secure || req.get('x-forwarded-proto') === 'https' ? 'https' : 'http';
      const isPublic = !host.includes('localhost') && !host.includes('127.0.0.1');
      res.json({
    ...
    this.app.get('/api/security/events', this.requireAdminAuth.bind(this), (req: Request, res: Response) => {
      const limit = parseInt(req.query.limit as string) || 50;
      res.json(this.security.getSecurityEvents(limit));
    });

    // Get admin API key (only accessible locally)
    this.app.get('/api/security/admin-key', (req: Request, res: Response) => {
    ...
    this.app.get('/api/monitoring/health', (req: Request, res: Response) => {
      const now = Date.now();
      const uptime = now - (this as any).startTime;
      const chain = this.blockchain.getChain();
    ...
    this.app.get('/api/monitoring/uptime', (req: Request, res: Response) => {
      const uptime = Date.now() - (this as any).startTime;
    ...
    this.app.get('/api/wallet/:address/transactions', (req: Request, res: Response) => {
    ...
    // Wallet Full Details
    this.app.get('/api/wallet/:address/details', (req: Request, res: Response) => {
      const address = req.params.address;
    ...
    // Transactions
    this.app.post('/api/transaction/send', this.strictRateLimit.bind(this), (req: Request, res: Response) => {
    ...
    this.app.post('/api/validators/register', this.strictRateLimit.bind(this), (req: Request, res: Response) => {
    ...
    // Block Production
    this.app.post('/api/blockchain/produce', this.requireAdminAuth.bind(this), (req: Request, res: Response) => {
    ...
    // Smart Contracts
    this.app.post('/api/contract/deploy', this.strictRateLimit.bind(this), (req: Request, res: Response) => {
  ...
  private setupDEXRoutes(): void {
    ...
    this.app.post('/api/dex/token/mint', this.strictRateLimit.bind(this), (req: Request, res: Response) => {
      try {
    ...
    this.app.get('/api/dex/token/balance/:token/:address', (req: Request, res: Response) => {
    ...
    this.app.post('/api/dex/swap', (req: Request, res: Response) => {
      try {
        ...
        if (this.marketTracker) {
          this.marketTracker.recordSwap(
            trader, tokenIn, tokenOut, amountIn,
            result.amountOut, result.fee, result.pool.id,
            this.blockchain.getChainHeight()
          );
        }
    ...
    this.app.get('/api/token/market', (req: Request, res: Response) => {
      if (!this.marketTracker) { res.json({ error: 'Market tracker not initialized' }); return; }
      const ts = this.blockchain.getTokenSystem();
      res.json(this.marketTracker.getMarketStats('VCO', ts.getTotalSupply(), ts.getMaxSupply()));
    });

    this.app.get('/api/token/swaps', (req: Request, res: Response) => {
      if (!this.marketTracker) { res.json([]); return; }
      const limit = parseInt(req.query.limit as string) || 50;
      res.json(this.marketTracker.getSwapHistory(limit));
    });

    this.app.get('/api/token/price-history/:poolId', (req: Request, res: Response) => {
      if (!this.marketTracker) { res.json([]); return; }
