...
import { getPublicKeyFromPrivateKey } from '../crypto';
 ...
 */
export function getEvmAddress(publicKey: string): string {
...
export function toChecksumAddress(address: string): string {
  ...
  const hash = Buffer.from(keccak_256(Buffer.from(addr, 'utf-8'))).toString('hex');
  let result = '0x';
 ...
 */
function rlpDecode(data: Uint8Array): any {
  let offset = 0;
 ...
 */
function parseEthereumTransaction(rawTx: string): {
  nonce: number;
  gasPrice: bigint;
  gasLimit: bigint;
  to: string;
  ...
  try {
    ...
    let nonce: number, gasPrice: bigint, gasLimit: bigint, to: string, value: bigint, data: string, v: number, r: string, s: string;
    let chainId = VERDIS_CHAIN_ID;
    ...
    const txHash = keccak_256(unsignedPayload);
 ...
 */
function buildUnsignedPayload(txType: number, decoded: any[], chainId: number): Uint8Array {
 ...
 */
function rlpEncode(item: any): Uint8Array {
  const parts: Buffer[] = [];
  ...
  function encode(item: any): Buffer {
 ...
 */
function formatBlock(block: any, includeTxs: boolean = false): any {
  ...
  return {
    ...
    transactions: includeTxs 
      ...
      : (block.transactions || []).map((t: any) => '0x' + (t.id || '')),
    uncles: [],
  };
}
 ...
 */
function formatTransaction(tx: any, blockInfo?: any): any {
  return {
    ...
    type: '0x0',
    chainId: '0x' + VERDIS_CHAIN_ID.toString(16),
  };
}
 ...
 */
export function setupJsonRpc(app: Express, blockchain: Blockchain, walletManager: WalletManager): void {
  ...
  app.get('/rpc', (req: Request, res: Response) => {
    res.json({
      jsonrpc: '2.0',
      id: null,
...
async function handleRpc(
  ...
  if (Array.isArray(req.body)) {
    const results = req.body.map((r: any) => processMethod(r.method, r.params, r.id, blockchain, walletManager));
    res.json(results);
    return;
  }
...
function processMethod(
  ...
  try {
    switch (method) {
      case 'eth_chainId':
        return { ...response, result: '0x' + VERDIS_CHAIN_ID.toString(16) };

      case 'net_version':
        return { ...response, result: String(VERDIS_CHAIN_ID) };

      case 'web3_clientVersion':
        return { ...response, result: 'Verdis/v1.0.0' };

      case 'web3_sha3':
        return { ...response, result: '0x' + Buffer.from(keccak_256(Buffer.from(params[0].replace('0x', ''), 'hex'))).toString('hex') };
      ...
      case 'eth_getBalance': {
        ...
        for (const w of walletManager.getAllWallets()) {
          ...
          if (evm === address) { nativeAddr = w.address; break; }
          if (w.address.toLowerCase() === address) { nativeAddr = w.address; break; }
        }
      ...
      case 'eth_getTransactionCount': {
        ...
        for (const block of blockchain.getChain()) {
          for (const tx of block.transactions) {
            if (tx.from.toLowerCase() === nativeAddr.toLowerCase()) nonce++;
          }
        }
      ...
      case 'eth_getBlockByNumber': {
        ...
        if (!block) return { ...response, result: null };
      ...
      case 'eth_getTransactionByHash': {
        ...
        if (!receipt.tx) return { ...response, result: null };
      ...
      case 'eth_sendRawTransaction': {
        ...
        const to = parsed.to;
        const tx = walletManager.signTransaction(
          wallet,
          to,
          amountVrs,
          feeVrs > 0 ? feeVrs : 1,
          Date.now(),
          parsed.data !== '0x' ? parsed.data : undefined
        );
      ...
      case 'eth_getTransactionByBlockNumberAndIndex': {
        ...
        const block = blockchain.getBlockByIndex(blockNum);
        if (!block || !block.transactions[idx]) return { ...response, result: null };
