...
class SecurityManager {
    ...
    // === RATE LIMITING ===
    checkRateLimit(ip, strict = false) {
    ...
    isValidatorSlashed(address) {
        return this.slashedValidators.has(address);
    }
    getSlashedValidators() {
    ...
    validateAddress(address) {
        if (!address || typeof address !== 'string')
            return false;
        if (address.length < 10 || address.length > 64)
            return false;
        if (!address.startsWith('0x') && !address.startsWith('03') && !address.startsWith('02'))
            return false;
        return true;
    }
    ...
    // === SECURITY AUDIT LOG ===
    logEvent(type, severity, message, ip, address) {
        const event = {
            timestamp: Date.now(),
            type,
            severity,
