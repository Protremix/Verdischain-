...
class StackVM {
    ...
    run(bytecode) {
        ...
        while (this.pc < bytecode.length && !this.halted) {
            ...
            let gasCost = 1;
            if (opcode === exports.OPCODES.STORE ||
                opcode === exports.OPCODES.LOAD ||
                opcode === exports.OPCODES.SSTORE ||
                opcode === exports.OPCODES.SLOAD) {
                gasCost = 3;
            }
            else if (opcode === exports.OPCODES.CALL) {
                gasCost = 5;
            }
...
function compileContract(source) {
    ...
    for (const { tokens } of parsedInstructions) {
        ...
        else if (['JUMP', 'JUMPI', 'LOAD', 'STORE', 'SLOAD', 'SSTORE', 'LOG', 'EMIT', 'CALL'].includes(opStr) &&
            tokens.length > 1) {
            const arg = tokens[1];
            bytecode.push(exports.OPCODES.PUSH, parseArg(arg, labels));
