...
const dex_1 = require("../src/core/dex");
// Test framework
let passed = 0;
let failed = 0;
const failures = [];
...
async function main() {
    ...
    const tracker = new market_1.MarketTracker(dex);
    ...
    dex.depositToken('CARBON', '0xwallet1', 1000000);
    dex.depositToken('ECO', '0xwallet2', 2000000);
    dex.createPool('VRS', 'CARBON');
    dex.createPool('VRS', 'ECO');
    const liq1 = dex.addLiquidity('0xwallet1', 'VRS', 'CARBON', 100000, 50000);
    assert(liq1.lpTokens > 0, 'Liquidity added to VRS/CARBON pool');
    const liq2 = dex.addLiquidity('0xwallet2', 'VRS', 'ECO', 200000, 400000);
    assert(liq2.lpTokens > 0, 'Liquidity added to VRS/ECO pool');
    // ── Test 1: recordSwap ──
    section('Test 1: recordSwap');
    // Execute a real swap
    const swap1 = dex.swap('0xwallet1', 'CARBON', 'VRS', 10000, 0);
    assert(swap1.amountOut > 0, 'Swap executed successfully');
    tracker.recordSwap('0xwallet1', 'CARBON', 'VRS', 10000, swap1.amountOut, swap1.fee, swap1.pool.id, 1);
    ...
    assert(history[0].amountIn === 10000, 'Swap amountIn recorded correctly');
    ...
    dex.depositToken('CARBON', '0xwallet3', 100000);
    for (let i = 2; i <= 5; i++) {
        const swap = dex.swap('0xwallet3', 'CARBON', 'VRS', 5000, 0);
        tracker.recordSwap('0xwallet3', 'CARBON', 'VRS', 5000, swap.amountOut, swap.fee, swap.pool.id, i);
    }
    assert(tracker.getSwapHistory().length === 5, '5 swaps recorded after multiple trades');
    const stats1 = tracker.getMarketStats('VRS', 50000000000, 100000000000);
    assert(stats1.totalSwaps === 5, 'Total swap count = 5');
    assert(stats1.totalVolume === 10000 + 4 * 5000, 'Total volume = 30,000');
    assert(stats1.volume24h === 30000, '24h volume = 30,000 (all within 24h)');
    ...
    const stats = tracker.getMarketStats('VRS', 50000000000, 100000000000);
    ...
    assertApprox(stats.marketCap, stats.priceUSD * 50000000000, 1, 'Market cap = price * supply');
    assert(stats.circulatingSupply === 50000000000, 'Circulating supply correct');
    assert(stats.totalVolume === 30000, 'Total volume = 30,000');
    ...
    assert(vrsCarbonPool.volume24h === 30000, 'VRS/CARBON pool volume = 30,000');
    ...
    const fullHistory = tracker.getSwapHistory(10000);
    ...
    const allHistory = tracker.getSwapHistory(10000);
    assert(allHistory.length === fullHistory.length, 'getSwapHistory(10000) returns all swaps');
    ...
    const tracker2 = new market_1.MarketTracker(dex);
    ...
    assert(tracker2.getSwapHistory(10000).length === exported.swapHistory.length, 'Imported history matches export');
    const stats2 = tracker2.getMarketStats('VRS', 50000000000, 100000000000);
    ...
    const emptyDex = new dex_1.DEX();
    const emptyTracker = new market_1.MarketTracker(emptyDex);
    const emptyStats = emptyTracker.getMarketStats('VRS', 50000000000, 100000000000);
    ...
    const zeroPoolStats = emptyTracker.getMarketStats('VRS', 1000000, 2000000);
    ...
    try {
        ...
        await fetch(`${API}/api/dex/token/mint`, {
            ...
            body: JSON.stringify({ token: 'CARBON', address: trader, amount: 100000 })
        });
        ...
        const found = recentSwaps.some((s) => s.trader === trader && s.tokenIn === 'CARBON' && s.amountIn === 500);
