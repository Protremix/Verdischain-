  ...
  <div class="footer">
    ...
    <p style="margin-top:0.5rem">Built with @noble/secp256k1 and @noble/hashes. No mocks. No third-party dependencies.</p>
        ...
        this.app.get('/docs', (req, res) => {
            const docsPath = path_1.default.resolve(__dirname, '../web/api-docs.html');
            if (fs_1.default.existsSync(docsPath)) {
                res.sendFile(docsPath);
                return;
            }
            const docsAlt = path_1.default.resolve(__dirname, '../../src/web/api-docs.html');
            if (fs_1.default.existsSync(docsAlt)) {
                res.sendFile(docsAlt);
                return;
            }
    ...
    setupEcoRoutes() {
        ...
        // === BRIDGE ENDPOINTS ===
        this.app.post("/api/bridge/lock", this.strictRateLimit.bind(this), async (req, res) => {
            try {
                const { fromAddress, amount, bscAddress, privateKey } = req.body;
                if (!fromAddress || !amount || !bscAddress || !privateKey) {
                    return res.status(400).json({ error: "Missing required fields: fromAddress, amount, bscAddress, privateKey" });
                }
                if (amount <= 0) return res.status(400).json({ error: "Amount must be positive" });

                const balance = this.blockchain.getTokenSystem().getBalance(fromAddress);
                if (balance < amount) {
                    return res.status(400).json({ error: "Insufficient VRS balance", balance, required: amount });
                }
                ...
                const lockTx = {
                    ...
                    amount: amount,
                    nonce: Date.now(),
                    data: JSON.stringify({ action: "bridge_lock", mintId, bscAddress }),
                    timestamp: Date.now()
                };
                lockTx.id = crypto.createHash("sha256").update(JSON.stringify(lockTx)).digest("hex");
                lockTx.hash = lockTx.id;

                // Add to mempool
                this.blockchain.getMempool().addTransaction(lockTx, this.blockchain.getTokenSystem().getBalancesMap());

                // Save bridge lock record
                if (!this.bridgeLocks) this.bridgeLocks = new Map();
                this.bridgeLocks.set(mintId, {
                    mintId,
                    fromAddress,
                    bscAddress,
                    amount,
                ...
                res.json({
                    success: true,
                    mintId,
                    message: "VRS locked. Bridge operator will mint wVRS on BSC.",
                    lockTxHash: lockTx.hash,
                    amount,
                    bscAddress,
                    explorerUrl: "/api/transaction/" + lockTx.hash
                });
            } catch (e) {
        ...
        this.app.get("/api/bridge/lock/:mintId", (req, res) => {
            if (!this.bridgeLocks) this.bridgeLocks = new Map();
            const lock = this.bridgeLocks.get(req.params.mintId);
            if (!lock) return res.status(404).json({ error: "Lock not found" });
            res.json(lock);
        });

        this.app.get("/api/bridge/locks", (req, res) => {
            if (!this.bridgeLocks) this.bridgeLocks = new Map();
            const locks = Array.from(this.bridgeLocks.values());
            res.json({ total: locks.length, locks });
        });

        this.app.post("/api/bridge/redeem", this.requireAdminAuth.bind(this), (req, res) => {
            try {
                const { mintId, toAddress, amount } = req.body;
                if (!mintId || !toAddress || !amount) {
                    return res.status(400).json({ error: "Missing required fields: mintId, toAddress, amount" });
                }

                // Add VRS back to user on Verdis chain
                this.blockchain.getTokenSystem().addBalance(toAddress, amount);

                // Record redemption
                if (!this.bridgeLocks) this.bridgeLocks = new Map();
                const redeemId = require("crypto").randomBytes(32).toString("hex");
                this.bridgeLocks.set(redeemId, {
                    redeemId,
                    mintId,
                    toAddress,
                    amount,
                ...
                res.json({
                    success: true,
                    redeemId,
                    message: "VRS unlocked and sent to " + toAddress,
                    amount,
                    toAddress
                });
            } catch (e) {
        ...
        this.app.get("/api/bridge/stats", (req, res) => {
            if (!this.bridgeLocks) this.bridgeLocks = new Map();
            const locks = Array.from(this.bridgeLocks.values());
            const totalLocked = locks.filter(l => l.status === "locked").reduce((s, l) => s + l.amount, 0);
            const totalRedeemed = locks.filter(l => l.status === "redeemed").reduce((s, l) => s + l.amount, 0);
            res.json({
                totalLocks: locks.filter(l => l.status === "locked").length,
                totalRedeems: locks.filter(l => l.status === "redeemed").length,
                totalLockedAmount: totalLocked,
                totalRedeemedAmount: totalRedeemed,
                wvrsContract: "pending deployment",
                bscChainId: 56,
        ...
        this.app.post("/api/faucet/claim", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { address } = req.body;
                if (!address) return res.status(400).json({ error: "Address required" });
                const now = Date.now();
                const lastClaim = this.faucetClaims.get(address) || 0;
                const cooldown = 3600000;
                if (now - lastClaim < cooldown) {
                    const remaining = Math.ceil((cooldown - (now - lastClaim)) / 60000);
                    return res.status(429).json({ error: "Cooldown active", retryAfterMinutes: remaining });
                }
                this.blockchain.getTokenSystem().addBalance(address, 1000);
                this.faucetClaims.set(address, now);
                res.json({ success: true, address, amount: 1000, message: "1000 VRS sent", nextClaim: new Date(now + cooldown).toISOString() });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/faucet/status", (req, res) => {
            const address = req.query.address;
            if (address) {
                const lastClaim = this.faucetClaims.get(address) || 0;
                const remaining = Math.max(0, 3600000 - (Date.now() - lastClaim));
                res.json({ eligible: remaining === 0, remainingMinutes: Math.ceil(remaining / 60000) });
            } else { res.json({ amount: 1000, cooldownMinutes: 60, totalClaims: this.faucetClaims.size }); }
        });

        // === STAKING UI ===
        this.app.post("/api/staking/delegate", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { voter, validatorAddress, amount } = req.body;
                if (!voter || !validatorAddress || !amount) return res.status(400).json({ error: "voter, validatorAddress, amount required" });
                const balance = this.blockchain.getTokenSystem().getBalance(voter);
                if (balance < amount) return res.status(400).json({ error: "Insufficient balance", balance, required: amount });
                this.blockchain.getTokenSystem().subtractBalance(voter, amount);
                this.blockchain.getConsensus().vote(voter, validatorAddress, amount, this.blockchain.getTokenSystem());
                res.json({ success: true, voter, validator: validatorAddress, amount, message: "Delegated " + amount + " VRS" });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.post("/api/staking/undelegate", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { voter, validatorAddress, amount } = req.body;
                if (!voter || !validatorAddress || !amount) return res.status(400).json({ error: "voter, validatorAddress, amount required" });
                const stakes = this.blockchain.getConsensus().stakes || [];
                const stake = stakes.find(s => s.voter === voter && s.validator === validatorAddress);
                if (!stake || stake.amount < amount) return res.status(400).json({ error: "Insufficient stake" });
                stake.amount -= amount;
                const validator = this.blockchain.getConsensus().getAllValidatorsList().find(v => v.address === validatorAddress);
                if (validator) validator.votes -= amount;
                this.blockchain.getTokenSystem().addBalance(voter, amount);
                res.json({ success: true, voter, validator: validatorAddress, amount, message: "Undelegated " + amount + " VRS" });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/staking/delegations/:address", (req, res) => {
            const stakes = this.blockchain.getConsensus().stakes || [];
            const delegations = stakes.filter(s => s.voter === req.params.address);
            res.json({ address: req.params.address, delegations, total: delegations.length });
        });
        this.app.get("/api/staking/rewards/:address", (req, res) => {
            const stakes = this.blockchain.getConsensus().stakes || [];
            const delegations = stakes.filter(s => s.voter === req.params.address);
            const totalStaked = delegations.reduce((sum, s) => sum + s.amount, 0);
            res.json({ address: req.params.address, totalStaked, delegations: delegations.length, estimatedDailyReward: totalStaked * 0.0001 });
        });

        // === LIQUIDITY PROVISION ===
        this.app.post("/api/dex/liquidity/add", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { provider, tokenA, tokenB, amountA, amountB } = req.body;
                if (!provider || !tokenA || !tokenB || !amountA || !amountB) return res.status(400).json({ error: "Missing fields" });
                const result = this.dex.addLiquidity(provider, tokenA, tokenB, amountA, amountB);
                res.json({ success: true, ...result });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.post("/api/dex/liquidity/remove", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { provider, poolId, lpAmount } = req.body;
                if (!provider || !poolId || !lpAmount) return res.status(400).json({ error: "Missing fields" });
                const result = this.dex.removeLiquidity(provider, poolId, lpAmount);
                res.json({ success: true, ...result });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/dex/liquidity/:address", (req, res) => {
            const pools = this.dex.getAllPools();
            const positions = [];
            pools.forEach(pool => {
                if (pool.lpBalances) {
                    Object.entries(pool.lpBalances).forEach(([addr, amount]) => {
                        if (addr === req.params.address && amount > 0) {
                            positions.push({ poolId: pool.id, tokenA: pool.tokenA, tokenB: pool.tokenB, lpAmount: amount, reserveA: pool.reserveA, reserveB: pool.reserveB });
                        }
                    });
                }
            });
        ...
        // === TOKEN CREATION ===
        this.customTokens = [];
        this.app.post("/api/token/create", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { name, symbol, totalSupply, creator } = req.body;
                if (!name || !symbol || !totalSupply || !creator) return res.status(400).json({ error: "Missing fields" });
                const crypto = require("crypto");
                const tokenId = crypto.randomBytes(16).toString("hex");
                const token = { id: tokenId, name, symbol, totalSupply, creator, createdAt: Date.now() };
                this.customTokens.push(token);
                this.dex.createPool(symbol, "VRS", 0.003);
                res.json({ success: true, token, message: "Token " + symbol + " created" });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/token/list", (req, res) => { res.json(this.customTokens); });

        // === GOVERNANCE ===
        this.proposals = [];
        this.app.post("/api/governance/proposal", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { title, description, proposer, type } = req.body;
                if (!title || !description || !proposer) return res.status(400).json({ error: "Missing fields" });
                const crypto = require("crypto");
                const proposalId = crypto.randomBytes(16).toString("hex");
                const proposal = { id: proposalId, title, description, proposer, type: type || "improvement", status: "active", votesFor: 0, votesAgainst: 0, voters: [], createdAt: Date.now(), endsAt: Date.now() + 259200000 };
        ...
        this.app.get("/api/governance/proposals", (req, res) => {
            ...
            res.json(status ? this.proposals.filter(p => p.status === status) : this.proposals);
        });
        ...
        this.app.post("/api/governance/vote", this.strictRateLimit.bind(this), (req, res) => {
            try {
                ...
                if (!proposalId || !voter || !vote) return res.status(400).json({ error: "Missing fields" });
                if (vote !== "for" && vote !== "against") return res.status(400).json({ error: "vote must be for or against" });
                const proposal = this.proposals.find(p => p.id === proposalId);
                if (!proposal) return res.status(404).json({ error: "Not found" });
                if (proposal.voters.includes(voter)) return res.status(400).json({ error: "Already voted" });
                const votingPower = this.blockchain.getTokenSystem().getBalance(voter);
                proposal.voters.push(voter);
                if (vote === "for") proposal.votesFor += votingPower || 1;
                else proposal.votesAgainst += votingPower || 1;
                res.json({ success: true, proposal, votingPower: votingPower || 1 });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });

        // === NFT SUPPORT ===
        this.nfts = [];
        this.app.post("/api/nft/mint", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { name, description, image, creator, category, carbonValue } = req.body;
                if (!name || !creator) return res.status(400).json({ error: "name, creator required" });
                const crypto = require("crypto");
                const tokenId = crypto.randomBytes(32).toString("hex");
                const nft = { id: tokenId, name, description: description || "", image: image || "", creator, owner: creator, category: category || "eco", carbonValue: carbonValue || 0, createdAt: Date.now(), transfers: [] };
                this.nfts.push(nft);
                res.json({ success: true, nft });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/nft/list", (req, res) => {
            let filtered = this.nfts;
            if (req.query.category) filtered = filtered.filter(n => n.category === req.query.category);
            if (req.query.owner) filtered = filtered.filter(n => n.owner === req.query.owner);
            res.json(filtered);
        });
        this.app.get("/api/nft/:id", (req, res) => {
            const nft = this.nfts.find(n => n.id === req.params.id);
            if (!nft) return res.status(404).json({ error: "NFT not found" });
            res.json(nft);
        });
        this.app.post("/api/nft/transfer", this.strictRateLimit.bind(this), (req, res) => {
            try {
                const { nftId, from, to } = req.body;
                const nft = this.nfts.find(n => n.id === nftId);
                if (!nft) return res.status(404).json({ error: "NFT not found" });
                if (nft.owner !== from) return res.status(400).json({ error: "Not the owner" });
                nft.owner = to;
                nft.transfers.push({ from, to, timestamp: Date.now() });
                res.json({ success: true, nft });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });

        // === EXPLORER DETAIL PAGES ===
        this.app.get("/api/explorer/block/:height", (req, res) => {
            const block = this.blockchain.getBlockByIndex(parseInt(req.params.height));
            if (!block) return res.status(404).json({ error: "Block not found" });
            res.json(block);
        });
        this.app.get("/api/explorer/tx/:hash", (req, res) => {
            ...
            for (const block of blocks) {
                if (block.transactions) {
                    const tx = block.transactions.find(t => t.id === req.params.hash || t.hash === req.params.hash);
                    if (tx) return res.json({ ...tx, blockHeight: block.header.index, blockHash: block.hash });
                }
            }
            res.status(404).json({ error: "Transaction not found" });
        });
        this.app.get("/api/explorer/address/:address", (req, res) => {
            const address = req.params.address;
            const balance = this.blockchain.getTokenSystem().getBalance(address);
            const validator = this.blockchain.getConsensus().getAllValidatorsList().find(v => v.address === address);
            const stakes = (this.blockchain.getConsensus().stakes || []).filter(s => s.voter === address);
            const blocks = this.blockchain.getChain();
            const txs = [];
            blocks.forEach(b => {
                if (b.transactions) b.transactions.forEach(t => {
                    if (t.from === address || t.to === address) txs.push({ ...t, blockHeight: b.header.index });
                });
            });
            res.json({ address, balance, isValidator: !!validator, validatorInfo: validator, stakes, transactions: txs.slice(-20), totalTransactions: txs.length });
        });
        ...
        this.app.get("/api/docs", (req, res) => {
            res.json({
                ...
                baseUrl: "https://verdischain.com", rpcUrl: "https://rpc.verdischain.com",
                endpoints: {
        ...
        // === NOTIFICATIONS ===
        this.notifications = [];
        this.app.post("/api/notifications/subscribe", (req, res) => {
            try {
                const { address, telegram, email } = req.body;
                const crypto = require("crypto");
                const subId = crypto.randomBytes(16).toString("hex");
                this.notifications.push({ id: subId, address, telegram, email, createdAt: Date.now(), active: true });
                res.json({ success: true, subId, message: "Subscribed to notifications" });
            } catch (e) { res.status(500).json({ error: e.message }); }
        });
        this.app.get("/api/notifications/subscriptions", (req, res) => { res.json(this.notifications); });
