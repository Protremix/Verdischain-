...
4. Economic logic errors (reward distribution, slashing, staking)
...
"""
    ...
    try:
        ...
        if "content" in resp:
            ...
            print(f"ERROR: {label} - {result.stdout[:500]}")
            return None
    except Exception as e:
        print(f"ERROR: {label} - {e}")
        return None

code = open("pallets/pallets_dpos_src_lib.txt").read()
mid = len(code) // 2
while mid < len(code) and code[mid] != '\n':
    mid += 1

call_claude(code[:mid], "audit_dpos_part1.md")
call_claude(code[mid:], "audit_dpos_part2.md")
