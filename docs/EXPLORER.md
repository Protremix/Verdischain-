...
* **Live Network Stats Bar:** Real-time block height, finalized height, TPS (transactions per second), epoch slot, total VRS circulating supply, active validators.
...
* **Summary Cards:** Total Supply (100B VRS), Market Cap, Active Accounts, Network Hashrate / VRF Slot execution, Eco Carbon Offsets total.
...
* **Transaction Table:** Displays Tx Hash, Status (`Success` / `Failed`), Block Number, Origin SS58 Address, Target Address, Value in VRS, and Transaction Fee.
...
* **Validator Metrics:**
  ...
  * **Total Stake:** Self-stake + Nominated stake in VRS.
  * **Commission Rate (%)**
...
* **Liquidity Pools Overview:** Lists native automated market maker trading pairs (e.g., `VRS/vUSDT`, `VRS/vCARBON`).
* **Pool Metrics:**
...
* **Distribution Graph:** Displays the 100B VRS token distribution.
* **Rich List / Top Holders Table:** Ranks account addresses by VRS balance, percentage of total supply, and account label (e.g., Treasury, Team Vesting, AMM Pool).
