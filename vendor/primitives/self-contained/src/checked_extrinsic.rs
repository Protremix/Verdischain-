...
use sp_runtime::traits::Pipeline;
	...
	fn validate<U: ValidateUnsigned<Call = Self::Call>>(
		...
		match &self.signed {
			GenericDelegated(format) => match format {
				...
				ExtrinsicFormat::General(ref variant) => {
					// sdk 48 folds the extension version into ExtensionVariant; read it back
					// through PipelineVersion rather than from a separate field.
					let version = sp_runtime::traits::PipelineVersion::version(variant);
					match variant {
						sp_runtime::traits::ExtensionVariant::V0(ref extension) => extension
							.validate_only(None.into(), &self.function, info, len, source, version)
							.map(|x| x.0),
						// ExtensionOtherVersions is InvalidVersion (an uninhabited enum) for
						// this runtime: only the v0 pipeline exists, so this value cannot be
						// constructed. Discharge it with a zero-arm match, which the compiler
						// verifies rather than a runtime panic.
						sp_runtime::traits::ExtensionVariant::Other(extension) => match *extension {},
					}
				}
			},
	...
	fn apply<U: ValidateUnsigned<Call = Self::Call>>(
		...
		match self.signed {
			CheckedSignature::GenericDelegated(format) => match format {
				...
				ExtrinsicFormat::General(variant) => {
					let version = sp_runtime::traits::PipelineVersion::version(&variant);
					match variant {
						sp_runtime::traits::ExtensionVariant::V0(extension) => extension
							.dispatch_transaction(None.into(), self.function, info, len, version),
						// Unreachable by construction - see the note in validate().
						sp_runtime::traits::ExtensionVariant::Other(extension) => match extension {},
					}
				}
			},
