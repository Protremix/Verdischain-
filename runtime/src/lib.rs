...
/// Blocks dangerous calls (Sudo) while allowing all normal operations.
/// Sudo calls are only allowed via root origin (EnsureRoot), not public transactions.
pub struct VerdisBaseCallFilter;
impl frame_support::traits::Contains<RuntimeCall> for VerdisBaseCallFilter {
    fn contains(call: &RuntimeCall) -> bool {
        match call {
            // Block sudo from public transactions — only root can dispatch
...
parameter_types! {
    ...
    pub const ValidatorCount: u32 = 6;
...
parameter_types! {
    ...
    pub const CreateTokenDeposit: u64 = 100_000_000_000; // 100 VRS (in smallest unit)
