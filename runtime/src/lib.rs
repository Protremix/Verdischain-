...
use frame_support::{
    ...
    traits::{ConstU32, ConstU64, ConstBool, Everything, Get, Randomness},
    weights::{
...
parameter_types! {
    pub const DepositPerItem: Balance = UNITS;
...
// === EVM Pallet Configuration ===
parameter_types! {
    pub const EvmChainId: u64 = 909;
    pub const EvmMaxCodeSize: u32 = 24576;
}

impl pallet_verdis_evm::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type ChainId = EvmChainId;
    type MaxCodeSize = EvmMaxCodeSize;
    type WeightInfo = ();
}
...
type Executive = frame_executive::Executive<Runtime, Block, frame_system::ChainContext<Runtime>, Runtime, AllPalletsWithSystem>;
...
/// Transaction extension data for transactions (frame v48)
...
pub const MaxEncodedLen: usize = 16777216; // 16 MiB

/// (Old UncheckedExtrinsic replaced above with frame v48 version)
...
impl_runtime_apis! {
    ...
    impl sp_consensus_babe::BabeApi<Block> for Runtime {
        fn configuration() -> sp_consensus_babe::BabeConfiguration {
            let _authorities = Babe::authorities();
            
            sp_consensus_babe::BabeConfiguration {
                slot_duration: Babe::slot_duration(),
                epoch_length: <Runtime as pallet_babe::Config>::EpochDuration::get(),
                c: (1, 4),
                authorities: Babe::authorities().into_iter().map(|x| x).collect(),
    ...
    impl sp_genesis_builder::GenesisBuilder<Block> for Runtime {
        ...
        fn preset_names() -> Vec<sp_genesis_builder::PresetId> {
            vec![
                sp_genesis_builder::DEV_RUNTIME_PRESET.to_owned(),
                sp_genesis_builder::LOCAL_TESTNET_RUNTIME_PRESET.to_owned(),
            ]
        }
    }
    }
