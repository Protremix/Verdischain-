    ...
    C::Api: EcoRuntimeApi<Block>,
{
    fn get_total_co2_offset(&self) -> RpcResult<u64> {
        let at = self.client.info().best_hash;
        self.client.runtime_api().get_total_co2_offset(at).map_err(rpc_err)
    }

    fn get_total_trees_planted(&self) -> RpcResult<u32> {
        let at = self.client.info().best_hash;
        self.client.runtime_api().get_total_trees_planted(at).map_err(rpc_err)
    }

    fn get_total_credits_retired(&self) -> RpcResult<u64> {
        let at = self.client.info().best_hash;
        self.client.runtime_api().get_total_credits_retired(at).map_err(rpc_err)
    }

    fn get_carbon_credit_count(&self) -> RpcResult<u32> {
        let at = self.client.info().best_hash;
        self.client.runtime_api().get_carbon_credit_count(at).map_err(rpc_err)
    }

    fn get_reforest_project_count(&self) -> RpcResult<u32> {
        let at = self.client.info().best_hash;
        self.client.runtime_api().get_reforest_project_count(at).map_err(rpc_err)
    }
