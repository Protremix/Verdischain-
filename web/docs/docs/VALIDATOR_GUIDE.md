...
| **Minimum Validator Self-Stake**| `100,000 VRDX` |
...
1. **Stash Account:** Holds the validator's primary funds (`100,000+ VRDX` self-stake). Kept in cold storage or secure wallet.
...
# Output Example:
# Secret seed:       REDACTED_KEY
# Public key (hex):  REDACTED_KEY
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": "REDACTED_KEY4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d"
}
```
...
2. Call `session.setKeys(keys, proof = 0x00)`:
   ...
   * `proof`: `0x00`.
3. Call `dpos.bond(amount = 100000 VRDX, payee = Staked)` to bond your minimum self-stake.
