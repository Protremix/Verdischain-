...
fn gas_cost(opcode: u8) -> u64 {
    match opcode {
        ...
        0x49 => 3,             // BLOBHASH
        0x4A => 2,             // BLOBBASEFEE
        0x30..=0x48 => 2,    // Environment (all other, cheap)
        0x50 => 2,            // POP
        0x51 => 3,            // MLOAD
        0x52 => 3,            // MSTORE
        0x53 => 3,            // MSTORE8
...
fn is_valid_jumpdest(_code: &[u8], pos: usize, jumpdests: &[usize]) -> bool {
...
pub fn execute<H: EvmHost>(ctx: &mut ExecutionContext, host: &mut H) -> ExecResult {
    ...
    let mut transient: Vec<(H160, H256, H256)> = Vec::new();
    ...
    loop {
        ...
        match opcode {
            ...
            0x3B => { let _addr = pop!(); push!(U256::zero()); pc += 1; }
            0x3C => {
            ...
            0x49 => { // BLOBHASH — returns 0 (no blobs in Verdis)
                let _index = pop!();
                push!(U256::zero());
                pc += 1;
            }
            0x4A => { // BLOBBASEFEE — returns 0 (no blob fee in Verdis)
                push!(U256::zero());
                pc += 1;
            }
            ...
            0x5C => { // TLOAD — load from transient storage
                let key = pop!();
                let key_h = H256::from_slice(&key.to_big_endian());
                let val = transient.iter().find(|(a, k, _)| *a == ctx.address && *k == key_h).map(|(_, _, v)| *v).unwrap_or(H256::zero());
                push!(U256::from_big_endian(val.as_bytes()));
                pc += 1;
            }
            0x5D => { // TSTORE — store to transient storage
                ...
                let value = pop!();
                let key_h = H256::from_slice(&key.to_big_endian());
                let val_h = H256::from_slice(&value.to_big_endian());
                if let Some(entry) = transient.iter_mut().find(|(a, k, _)| *a == ctx.address && *k == key_h) {
                    entry.2 = val_h;
                } else {
                    transient.push((ctx.address, key_h, val_h));
                }
                pc += 1;
            }
            0x5E => { // MCOPY — copy memory region
                let dest = pop!();
                let src = pop!();
                let len = pop!();
                let dest_off: usize = dest.try_into().unwrap_or(0);
                let src_off: usize = src.try_into().unwrap_or(0);
                let copy_len: usize = len.try_into().unwrap_or(0);
                if copy_len > 0 {
                    let data = match memory.load(src_off, copy_len) {
                        Ok(d) => d,
                        Err(e) => return ExecResult::Failed { error: e, gas_used: ctx.gas_used },
                    };
                    if let Err(e) = memory.store(dest_off, &data) {
                        return ExecResult::Failed { error: e, gas_used: ctx.gas_used };
                    }
                }
                pc += 1;
            ...
            // System - CREATE and CALL opcodes
            0xF0 => {
...
pub fn opcode_count() -> usize {
    ...
    // STOP: 1 (0x00)
    // Cancun opcodes: 5 (0x49, 0x4A, 0x5C, 0x5D, 0x5E)
    // Total: 137 + 5 = 142
    142
}
