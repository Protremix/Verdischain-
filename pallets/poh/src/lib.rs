...
pub mod pallet {
    ...
    pub type PohHashes<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        BlockNumberFor<T>,
        [u8; 32],
    ...
    pub enum Event<T: Config> {
        ...
        BlockStamped { block_number: BlockNumberFor<T>, hash: [u8; 32] },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn record_block(origin: OriginFor<T>) -> DispatchResult {
            ...
            Self::deposit_event(Event::BlockStamped {
                block_number,
                hash,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn tick() -> [u8; 32] {
            ...
            let new_hash =
                Self::calculate_hash(&config.last_hash, &config.seed, config.tick_count);
