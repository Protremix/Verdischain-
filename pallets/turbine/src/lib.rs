...
pub mod pallet {
    ...
    #[pallet::storage] pub type TurbineTotalShards<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type TurbineTotalBlocks<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type TurbineTreeDepth<T> = StorageValue<_, u32, ValueQuery>;
    #[pallet::storage] pub type TurbineValidatorCount<T> = StorageValue<_, u32, ValueQuery>;
    #[pallet::storage] pub type BlockShardCount<T> = StorageMap<_, Twox64Concat, u32, u32, ValueQuery>;
    ...
    #[pallet::error] pub enum Error<T> { MaxShardsExceeded, InvalidShardIndex, NoValidators }
    #[pallet::call]
    impl<T: Config> Pallet<T> {
        #[pallet::weight(0)]
        #[pallet::call_index(0)]
        pub fn register_shard(origin: OriginFor<T>, block_number: u32, shard_index: u32, total_shards: u32) -> DispatchResult {
            ...
            Self::deposit_event(Event::ShardPropagated { shard_id: shard_index, block_number });
            Ok(())
        }
        #[pallet::weight(0)]
        #[pallet::call_index(1)]
        pub fn rebuild_tree(origin: OriginFor<T>, validator_count: u32) -> DispatchResult {
            ...
            Self::deposit_event(Event::TreeRebuilt { depth, validator_count });
            Ok(())
        }
        #[pallet::weight(0)]
        #[pallet::call_index(2)]
        pub fn mark_block_propagated(origin: OriginFor<T>, block_number: u32) -> DispatchResult {
            ...
            Self::deposit_event(Event::BlockSharded { block_number, shard_count: sc });
            Ok(())
        }
    }
    impl<T: Config> Pallet<T> {
        fn calc_depth(count: u32, fanout: u32) -> u32 {
            ...
            let mut d = 1; let mut n = fanout;
            while n < count { n *= fanout; d += 1; }
            d
        }
    }
}
