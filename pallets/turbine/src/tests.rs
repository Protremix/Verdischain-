...
fn register_shard_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::register_shard(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 1, 0, 10
        ));
...
fn rebuild_tree_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::rebuild_tree(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 64
        ));
...
fn mark_block_propagated_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::mark_block_propagated(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 1
        ));
...
fn max_shards_exceeded() {
    new_test_ext().execute_with(|| {
        assert!(Pallet::<Test>::register_shard(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 1, 0, 999
        ).is_err());
    });
}
