...
fn create_batch_parallel_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::create_batch(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 10, false
        ));
...
fn create_batch_sequential_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::create_batch(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 10, true
        ));
...
fn report_execution_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::create_batch(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 5, false
        ));
        assert_ok!(Pallet::<Test>::report_execution(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 0, 5000, 5
        ));
...
fn report_conflict_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::report_conflict(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 0, 1, 2
        ));
