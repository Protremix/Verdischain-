...
pub mod pallet {
    ...
    #[pallet::storage] pub type SealevelTotalBatches<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelParallelBatches<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelSequentialBatches<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelTotalTxs<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelAvgComputeUnits<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelConflicts<T> = StorageValue<_, u64, ValueQuery>;
    #[pallet::storage] pub type SealevelParallelizationRate<T> = StorageValue<_, u32, ValueQuery>;
    #[pallet::storage] pub type NextBatchId<T> = StorageValue<_, u32, ValueQuery>;
    #[pallet::storage] pub type BatchParallel<T> = StorageMap<_, Twox64Concat, u32, bool, ValueQuery>;
    #[pallet::storage] pub type BatchComputeUnits<T> = StorageMap<_, Twox64Concat, u32, u64, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        ConflictDetected { batch_id: u32, tx1: u32, tx2: u32 },
    }
    #[pallet::error] pub enum Error<T> { BatchNotFound, ComputeBudgetExceeded, MaxBatchSizeExceeded }
    #[pallet::call]
    impl<T: Config> Pallet<T> {
        #[pallet::weight(0)]
        #[pallet::call_index(0)]
        pub fn create_batch(origin: OriginFor<T>, tx_count: u32, has_conflicts: bool) -> DispatchResult {
            ...
            Self::deposit_event(Event::BatchCreated { batch_id, tx_count, parallel });
            Ok(())
        }
        #[pallet::weight(0)]
        #[pallet::call_index(1)]
        pub fn report_execution(origin: OriginFor<T>, batch_id: u32, compute_units: u64, tx_count: u32) -> DispatchResult {
            let _ = ensure_signed(origin)?;
        ...
        #[pallet::weight(0)]
        #[pallet::call_index(2)]
        pub fn report_conflict(origin: OriginFor<T>, batch_id: u32, tx1: u32, tx2: u32) -> DispatchResult {
            let _ = ensure_signed(origin)?;
