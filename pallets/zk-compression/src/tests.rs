...
fn create_tree_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::create_tree(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 10
        ));
...
fn compress_account_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Pallet::<Test>::compress_account(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 0, 256
        ));
...
fn verify_proof_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Pallet::<Test>::verify_proof(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 0, 0, true
        ));
    });
}
...
fn max_depth_exceeded() {
    new_test_ext().execute_with(|| {
        assert!(Pallet::<Test>::create_tree(
            frame_system::RawOrigin::Signed(sp_core::crypto::AccountId32::from([0xff; 32])).into(), 999
        ).is_err());
    });
}
