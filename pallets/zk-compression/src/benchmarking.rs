...
mod benches {
    ...
    fn compress_account() {
        ...
        #[extrinsic_call]
        compress_account(RawOrigin::Signed(caller.clone()), 0, 1024);
    ...
    fn verify_proof() {
        let caller: T::AccountId = whitelisted_caller();
        let _ = Pallet::<T>::create_tree(RawOrigin::Signed(caller.clone()).into(), 20);
        let _ = Pallet::<T>::compress_account(RawOrigin::Signed(caller.clone()).into(), 0, 1024);

        #[extrinsic_call]
        verify_proof(RawOrigin::Signed(caller.clone()), 0, 0, [0u8; 32]);
    }
