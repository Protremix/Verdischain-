...
pub mod pallet {
    ...
    pub type PendingForwards<T: Config> = StorageMap<_, Twox64Concat, [u8; 32], ForwardedTransaction>;
    ...
    pub enum Event<T: Config> {
        TransactionForwarded { tx_hash: [u8; 32], to_validator: Vec<u8> },
        TransactionIncluded { tx_hash: [u8; 32], block_number: u32 },
        TransactionExpired { tx_hash: [u8; 32] },
        StatsUpdated { total_forwarded: u64, success_rate: u32 },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn forward_transaction(
            ...
            ensure!(!PendingForwards::<T>::contains_key(tx_hash), Error::<T>::AlreadyForwarded);
            ...
            Self::deposit_event(Event::TransactionForwarded { tx_hash, to_validator });
            Ok(())
        }
        ...
        pub fn mark_included(
            ...
            let mut tx = PendingForwards::<T>::get(tx_hash).ok_or(Error::<T>::TransactionNotFound)?;
            ...
            let new_avg = if stats.total_included == 1 {
                ...
                (stats.avg_forward_time_ms * (stats.total_included - 1) + forward_time_ms) / stats.total_included
            };
            stats.avg_forward_time_ms = new_avg;
            GulfStreamStatsStorage::<T>::put(stats);

            Self::deposit_event(Event::TransactionIncluded { tx_hash, block_number });
            Ok(())
        }
