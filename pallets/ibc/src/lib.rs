...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ClientCreated { client_id: u32, chain_id: u32 },
        ConnectionOpened { connection_id: u32, client_id: u32 },
        ChannelOpened { channel_id: u32, connection_id: u32, ordering: u8 },
        ChannelClosed { channel_id: u32 },
        PacketSent { channel_id: u32, sequence: u64, source_port: Vec<u8> },
        PacketReceived { channel_id: u32, sequence: u64, dest_port: Vec<u8> },
        PacketAcknowledged { channel_id: u32, sequence: u64 },
        PacketTimedOut { channel_id: u32, sequence: u64 },
        TransferInitiated { sender: T::AccountId, receiver: Vec<u8>, amount: u128, denom: Vec<u8>, channel_id: u32 },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn open_connection(
            ...
            let client = IbcClients::<T>::get(client_id)
        ...
        pub fn open_channel(
            ...
            let connection = IbcConnections::<T>::get(connection_id)
                .ok_or(Error::<T>::ConnectionNotFound)?;
            ensure!(connection.state == 3, Error::<T>::ConnectionNotOpen);
            ensure!(port_id.len() as u32 <= T::MaxPortIdLen::get(), Error::<T>::PortIdTooLong);
            ...
            Self::deposit_event(Event::ChannelOpened { channel_id, connection_id, ordering });
            Ok(())
        }
        ...
        pub fn send_packet(
            ...
            ensure!(data.len() as u32 <= T::MaxPacketDataLen::get(), Error::<T>::PacketDataTooLarge);
            ...
            Self::deposit_event(Event::PacketSent { channel_id, sequence, source_port });
            Ok(())
        }
        ...
        pub fn recv_packet(
            ...
            Self::deposit_event(Event::PacketReceived { channel_id, sequence, dest_port });
            Ok(())
        }
        ...
        pub fn acknowledge_packet(
            ...
            Self::deposit_event(Event::PacketAcknowledged { channel_id, sequence });
            Ok(())
        }
        ...
        pub fn timeout_packet(
            ...
            let packet = IbcPackets::<T>::get((channel_id, sequence))
                .ok_or(Error::<T>::ChannelNotFound)?;
            ...
            IbcPackets::<T>::remove((channel_id, sequence));
            Self::deposit_event(Event::PacketTimedOut { channel_id, sequence });
            Ok(())
        }
